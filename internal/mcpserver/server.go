...
func New(service Service) *mcp.Server {
	...
	mcp.AddTool(server, &mcp.Tool{Name: "wiki_search", Description: "Search an installed offline wiki snapshot. Full-text results use a bounded 250-candidate BM25 retrieval funnel with title, term-coverage, and phrase-proximity relevance signals; total is the deduplicated size of that funnel. Exact titles and redirects resolve to the canonical page as a hard first tier, and duplicate aliases are collapsed. Results include canonical URLs, page IDs, match metadata, finite relative scores, and query-centered snippets. Encyclopedia articles are searched by default; set include_non_articles to include project, category, draft, talk, and other namespaces. Follow a relevant result with wiki_read using its page_id.", InputSchema: searchSchema, Annotations: readOnlyAnnotations(false)}, func(ctx context.Context, _ *mcp.CallToolRequest, in searchInput) (*mcp.CallToolResult, model.SearchResult, error) {
