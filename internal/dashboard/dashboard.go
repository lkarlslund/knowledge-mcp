...
type Service interface {
	...
	BrowseAvailable(context.Context, string, string, string, bool, int, int, bool) (model.AvailableResult, error)
...
func Handler(service Service) http.Handler {
	...
	mux.HandleFunc("GET /api/dashboard/available", func(w http.ResponseWriter, r *http.Request) {
		...
		result, err := service.BrowseAvailable(r.Context(), query.Get("filter"), query.Get("language"), query.Get("source"), query.Get("hide_installed") == "true", offset, limit, query.Get("refresh") == "true")
		writeJSON(w, result, err)
	})
